...
## Recognize faces from videos, mark facial feature points in real time, and easily judge facial emotions
                        ...
                        if round(mouth_height >= 0.03):
                            if eye_hight >= 0.056:
                                cv2.putText(im_rd, "Amazing", (d.left(), d.bottom() + 20), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                            (0, 0, 255), 2, 4)
                                # Chinese_plot_box(im_rd, label='Amazing', x=[0, 0, d.left(), d.bottom() + 20], sizes=20, colour=None,
                                #                  line_thickness=None)
                            else:
                                cv2.putText(im_rd, "Happy", (d.left(), d.bottom() + 20), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                            (0, 0, 255), 2, 4)
                                # Chinese_plot_box(im_rd, label='Happy', x=[523, 719, 641, 823], sizes=20, colour=None,
                                #                  line_thickness=None)
                        ...
                        # 没有张嘴，可能是正常和生气，通过眉毛区分
                            ...
                            if self.brow_k <= -0.3:
                                cv2.putText(im_rd, "Angry", (d.left(), d.bottom() + 20), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                            (0, 0, 255), 2, 4)
                                # Chinese_plot_box(im_rd, label='Angry', x=[523, 719, 641, 823], sizes=20, colour=None,
                                #                  line_thickness=None)
                            else:
                                cv2.putText(im_rd, "Nature", (d.left(), d.bottom() + 20), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                            (0, 0, 255), 2, 4)
                                # Chinese_plot_box(im_rd, label='Nature', x=[523, 719, 641, 823], sizes=20, colour=None,
                                #                  line_thickness=None)
                # 标出人脸数
                cv2.putText(im_rd, "Face detected" + str(len(faces)), (20, 50), font, 0.6, (0, 0, 255), 1, cv2.LINE_AA)
            else:
                # 没有检测到人脸
